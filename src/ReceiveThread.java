		...
		try {
			while(!Thread.currentThread().isInterrupted()) {
				...
				if(receivePack.getLength() >= 7) {	// 7 Bytes is the header size
					...
					else if(header.op_code[0] == wiznet_header.REMOTE_CLEAR) {
						WIZnet_Header.Clear_Reply clear_reply = new WIZnet_Header.Clear_Reply();
						callback.receivedPacket(wiznet_header.REMOTE_CLEAR, clear_reply);
					}
					else if(header.op_code[0] == wiznet_header.REMOTE_LOCATE) {
						WIZnet_Header.Locate_Reply locate_reply = new WIZnet_Header.Locate_Reply();
						callback.receivedPacket(wiznet_header.REMOTE_LOCATE, locate_reply);
					}
					else {
					}
				}
