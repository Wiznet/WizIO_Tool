...
public class WIZDatagramSocket {
	...
	public void set_info(String host, byte[] packet, String set_pw) {
		...
		/* WizIO */
		if(packet[2] == 0x04 && packet[3] == 0x00 && packet[4] == 0x00) {
			WizIO_Config wizio_config = new WizIO_Config();
			wizio_config.setData(packet);
			mac = wizio_config.network_info_common.mac;
		}
	...
	public void firmware_upload(byte[] packet, byte[] serverIp, short serverPort, String fileName, String set_pw) {
		...
		if(packet[2] == 0x04 && packet[3] == 0x00 && packet[4] == 0x00) {
			...
			ip = String.format("%d.%d.%d.%d",
					(0xFF&wizio_config.network_info_common.local_ip[0]), (0xFF&wizio_config.network_info_common.local_ip[1]),
					(0xFF&wizio_config.network_info_common.local_ip[2]), (0xFF&wizio_config.network_info_common.local_ip[3]));
			mac = wizio_config.network_info_common.mac;
		}
	...
	public void clear(String host, byte[] dst_mac_address, String set_pw) {
		try {
			inet = InetAddress.getByName(host);
		} catch (UnknownHostException e1) {
			e1.printStackTrace();
		...
		WIZnet_Header wiznet_header = new WIZnet_Header();
		byte[] sendMsg = wiznet_header.clear(dst_mac_address, set_pw);
		if(host == "255.255.255.255") {
			wiznet_header.set_unicast(false, sendMsg);
		}
		else {
	...
	public void locate(String host, byte[] dst_mac_address, String set_pw) {
		try {
			inet = InetAddress.getByName(host);
		} catch (UnknownHostException e1) {
			e1.printStackTrace();
		...
		WIZnet_Header wiznet_header = new WIZnet_Header();
		byte[] sendMsg = wiznet_header.locate(dst_mac_address, set_pw);
		if(host == "255.255.255.255") {
			wiznet_header.set_unicast(false, sendMsg);
		}
		else {
