...
public class WIZnet_Header {
	...
	final byte REMOTE_CLEAR     		= (byte)0x10;
	final byte REMOTE_LOCATE       		= (byte)0x20;
	...
	public static class Clear_Reply {
		byte[] src_mac_address = new byte[6];
	}
	...
	public static class Locate_Reply {
		byte[] src_mac_address = new byte[6];
	}
	...
	*/
	...
	public byte[] clear(byte[] dst_mac_address, String set_pw) {
		Random random = new Random();
		Header header = new Header();
		...
		header.valid = (byte)(0x80 + random.nextInt(0x7F));
		header.unicast = 0;
		header.op_code[0] = REMOTE_CLEAR;
		header.op_code[1] = WIZNET_REQUEST;
		header.length = 23;
		System.arraycopy(dst_mac_address, 0, clear.dst_mac_address, 0, 6);
		clear.set_pw_len = (byte) set_pw.length();
		System.arraycopy(set_pw.getBytes(), 0, clear.set_pw, 0, set_pw.length());

		index = make_header(data, header);
		System.arraycopy(clear.dst_mac_address, 0, data, index, 6);
		index += 6;
		data[index++] = clear.set_pw_len;
		System.arraycopy(clear.set_pw, 0, data, index, 16);
		index += 16;
	...
	public byte[] locate(byte[] dst_mac_address, String set_pw) {
		Random random = new Random();
		Header header = new Header();
		...
		header.valid = (byte)(0x80 + random.nextInt(0x7F));
		header.unicast = 0;
		header.op_code[0] = REMOTE_LOCATE;
		header.op_code[1] = WIZNET_REQUEST;
		header.length = 23;
		System.arraycopy(dst_mac_address, 0, locate.dst_mac_address, 0, 6);
		locate.set_pw_len = (byte) set_pw.length();
		System.arraycopy(set_pw.getBytes(), 0, locate.set_pw, 0, set_pw.length());

		index = make_header(data, header);
		System.arraycopy(locate.dst_mac_address, 0, data, index, 6);
		index += 6;
		data[index++] = locate.set_pw_len;
		System.arraycopy(locate.set_pw, 0, data, index, 16);
		index += 16;
